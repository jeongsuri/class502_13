...
public class Ex02Test {
    ...
    void init(){
        ...
        //mailer = mock(Mailer.class); // = @Mock
        faker = new Faker(Locale.ENGLISH);
        //request = mock(HttpServletRequest.class); // =@Mock
    ...
    void test1(){
        ...
        then(mailer).should(only()).send(email); // send함수가 loginService.process 메서드에서 단 한번 호출되는지 테스트
    }
    ...
    void test2(){
        ...
        String email = request.getParameter("email"); //사용자가 직접 입력한 값

        ArgumentCaptor<String> captor = ArgumentCaptor.forClass(String.class);
        then(mailer).should().send(captor.capture());

        String usedEmail = captor.getValue(); //send메서드에서 매개변수로 사용한 값

        assertEquals(email, usedEmail); //일치여부 확인
    }
}
